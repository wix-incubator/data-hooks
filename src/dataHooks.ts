...
export function dataHooks<T extends DataHooks>(componentName: string): DataHooksGenerator<T> {
  return new Proxy({}, handler<T>(componentName));
}

function getStrOptions<Options extends { [key: string]: DataHookValue }>(options: Options): string[] {
